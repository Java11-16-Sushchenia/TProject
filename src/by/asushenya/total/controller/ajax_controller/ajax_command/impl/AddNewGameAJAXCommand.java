...
public class AddNewGameAJAXCommand implements IAJAXCommand{
	...
	public void execute(HttpServletRequest request, 
		...
		GameKind gameKind = GameKind.valueOf(request.getParameter(
										RequestParameterName.GAME_KIND)
										.toUpperCase());
		String firstTeamName =request.getParameter(
										RequestParameterName.FIRST_TEAM);
		String secondTeamName =request.getParameter(
										RequestParameterName.SECOND_TEAM);
		Timestamp gameDate = Timestamp.valueOf(request.getParameter(
										RequestParameterName.GAME_DATE));
		double k1 = Double.parseDouble(request.getParameter(
										RequestParameterName.K1));
		double kx = Double.parseDouble(request.getParameter(
										RequestParameterName.KX));
		double k2 = Double.parseDouble(request.getParameter(
										RequestParameterName.K2));
		...
		try{			
			serviceResponse = adminService.addNewGame(gameKind, 
													  firstTeamName, 
													  secondTeamName,
													  gameDate,
													  k1, 
			...
			throw new AJAXCommandException(
					  "can't add new game",e);
		}
